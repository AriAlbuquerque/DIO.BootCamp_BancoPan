...
public class ResumoPOO {
    ...
    PARADIGMA DE PROGRAMAÇÃO ORIENTADA A OBJETO: Modelo de desenvolvimento de aplicações, sendo este modelo construido acerca do conceito OBJETO;

    *PARADIGMA: Modelo ou exemplo a ser seguido;
    ...
    *** OBJETO---> É o que executa faz tudo acontecer, deve estar numa classe estática, é a representação de um conceito/entidade do mundo real, que pode ser física ou conceitual;
    ...
    * Definição: O POO(Programação Orientada á Objeto), é um paradigma de análise, projeto de programação d sistemas de software,
    baseado na composição e interação entre diversas unidades de software chamadas OBJETO;

    *Análise: Compreender e desenvolver forma com maior automação possível para o projeto;

    *Projeto: Pegar necessidades reais e modelá-las e criar um modelo computacional;
    ...
    *Eleva o nível de Abstração;
    ...
    @ Reuso: Capacidade de criar unidades de código a partir de outras já existentes á partir de abstração, ;
 ...
 *** Paradigma Estruturado(PE): Utilizado pela linguagem C tem uma representação mais simplista; foca em operações(funções) e dados;
    ...
    * Limpeza de lixo manual, aumenta a hipótese de erro no código;
...
************Estruturas básicas de OO*****************
    ...
    *** CLASSE: É estatica serve de molde para criar objetos, unidade mínima e básica de implementação de POO, estrutura que abstrai um conjunto de OBJETOS com características similares;
    Um Carro define o comportamento dos seus Objetos através de métodos e os estados possíveis destes através de atributos;
    ...
    *** ATRIBUTOS(Próprio peculiar a alguém ou algo): é o elemento de uma Classe responsãvel por DEFINIR sua ESTRUTURA DE DADOS;
    O conjunto destes será responsável por representar as suas características e fará parte dos objetos criados a partir da classe;
